...
#     print("Agent Response:", response)  # Debugging step

#     # Check if the response includes a tool call
#     if "function_call" in response.additional_kwargs:
#         return {"messages": messages + [response]}

#     # If no function_call, fall back to manual tool invocation
#     print("No tool call detected. Invoking tools manually.")
#     generated_code = response.content.strip("```python").strip("```")

#     # Determine the tool to invoke based on query
#     query = state["query"].lower()
#     if "plot" in query or "chart" in query or "graph" in query:
#         tool_response = execute_plot_tool(state["dataframe"], generated_code)
#     else:
#         tool_response = execute_code_tool(state["dataframe"], generated_code)

#     # Add the tool response as a FunctionMessage
#     tool_message = FunctionMessage(content=tool_response)
#     return {"messages": messages + [response, tool_message]}
...
def call_model_sim(state):
    response = AIMessage(
        content="",  # The content can remain empty since the tool call contains the details
        response_metadata={
            "tool_calls": [
                {
                    "name": "execute_plot_tool",
                    "args": {
                        "dataframe_json": state["dataframe"],
                        "generated_code": """
                        def plot_streams(df):
                            ...
                            # Simplified plot code for testing
