...
def create_system_message(state: AgentState):
    ...
    The function should accept a pd.DataFrame object with the same structure as {sample_df_head} as a parameter. 
    ...
    **IMPORTANT**: 
    - If the query requires analysis or visualization, call the appropriate tool by returning a `function_call`. 
    - Use the tool `execute_code_tool` for analysis or `execute_plot_tool` for visualization.
    - For a plot function, include code that generates a base64-encoded image.
    """
